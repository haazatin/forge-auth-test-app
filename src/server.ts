...
await migrate(pool);
const app = createApp({ pool, sessionSecret: required("SESSION_SECRET"), nodeEnv, baseUrl: process.env.BASE_URL ?? `http://localhost:${port}`, resetDelivery: process.env.PASSWORD_RESET_DELIVERY === "log" ? "log" : "screen" });
